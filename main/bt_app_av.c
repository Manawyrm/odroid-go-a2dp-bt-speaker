...
static short stereoAudioBuffer[16384];
static float Volume = 0.5f;
void bt_app_a2d_data_cb(const uint8_t *data, uint32_t len)
{
    size_t bytes_written;

    memcpy(stereoAudioBuffer, data, len); 
    ...
    int count = i2s_write(0, (const char *)stereoAudioBuffer, len, &bytes_written, portMAX_DELAY);
 

    //i2s_write(0, abuffer, len, &bytes_written, portMAX_DELAY);
    if (++s_pkt_cnt % 100 == 0) {
